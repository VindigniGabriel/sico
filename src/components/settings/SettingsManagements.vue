        ...
        <v-layout>
            <v-flex xs12>
                <v-toolbar flat color="white">
                    <v-toolbar-title class="hidden-sm-and-down primary--text" >Actualizar Gerencias Comerciales</v-toolbar-title>
                    <v-divider
                    class=""
                    inset
                    vertical
                    ...
                    <v-card>
                        ...
                        >
                            <v-card-title>
                                <span class="headline">Agregar Gerencia Comercial</span>
                            </v-card-title>
                            <v-card-text>
    ...
    mounted() {
        ...
        firebase.firestore().collection('directionsCommercial')
            .onSnapshot( snapshot => {
                this.dataDir = []
                snapshot.forEach(request => {
                    this.dataDir.push(request.data().name)
                })
            })
    },
    methods: {
        save(){
            firebase.firestore()
                ...
                .then(snapshot => {
                    if(snapshot.empty){
                          var id = firebase.firestore().collection("managementsCommercial").doc()
                            this.data = {
                                name: this.name,
                                holder: this.holder,
                                direction: this.nameDir
                            }
                            firebase.firestore()
                                .collection("managementsCommercial")
                                .doc(id.id)
                                .set(this.data)
                        this.$alertify.success("Se ha agregado una Gerencia Comercial")
                    }else{
                        this.$alertify.error("El nombre de la Gerencia Comercial ya existe")
                    }
                })
            this.close()
        },
